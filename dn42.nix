...
let
  ...
  isRouter = machine: machine.kioubitDn42.enable || machine.routedbitsDn42.enable;
  ...
  connectedMachines = builtins.filter (
    otherMachine: (isServer thisMachine) || (isServer otherMachine)
  ) otherMachines;
  ...
  networks = lib.imap (index: otherMachine: {
  ...
  config = lib.mkIf peeringEnabled {
    networking = {
      firewall.interfaces = lib.mkMerge (
        builtins.map (network: { ${network.name}.allowedTCPPorts = [ 12411 ]; }) networks
      );
    };
        ...
        ''
          ...
          protocol pipe bgp6_to_master6 {
                  ...
                  export filter {
                          ...
                          if ifname = "antibuilding" ${
                            lib.concatMapStrings (
                              machine: ''|| ifname = "antibuilding${builtins.toString machine.address}" ''
                            ) machines
                          } then {
